...
fn err_process_not_started() -> Box<dyn Error> {
    "进程未启动".into()
}

// mutex
pub(crate) struct CmdJob<T: Clone>
{
    cmd: Command,
    result: Option<Child>,
    working_dir: String,
    ...
    finished: Mutex<mpsc::Receiver<Empty>>,
    provider: Box<dyn MirrorProvider<ContextStoreVal=T>>,
    ret_err: Option<String>,
}

impl <T: Clone> CmdJob<T> {
    pub(crate) fn new(provider: Box<dyn MirrorProvider<ContextStoreVal=T>>,
                      cmd_and_args: Vec<String>,
                      working_dir: &str,
                      env: &HashMap<String, String>) -> Self
    {
        let mut cmd_job: CmdJob<T>;
...
fn new_environ(env: &HashMap<String, String>, inherit: bool) -> HashMap<String, String> {
