...
mod tests {
    ...
    fn test_zfs_hook() {
        ...
        // working_dir_do_not_exist(provider, tmp_dir)
        working_dir_is_not_a_mount_point(provider, tmp_dir)
    }
    fn working_dir_do_not_exist(provider: CmdProvider, tmp_dir: TempDir){
        tmp_dir.close().unwrap();
        let hook = ZfsHook::new("test_pool".into());
        let err = hook.pre_job(provider.working_dir(), provider.name());
        assert!(err.is_err());
    }
    fn working_dir_is_not_a_mount_point(provider: CmdProvider, tmp_dir: TempDir){
        let hook = ZfsHook::new("test_pool".into());
        let err = hook.pre_job(provider.working_dir(), provider.name());
        assert!(err.is_err());
    }
